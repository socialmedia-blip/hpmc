...
export default function AdminSiteVisit() {
  ...
  const [filteredContacts, setFilteredContacts] = useState<Vendor[]>([]);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`${API_BASE}/vendor/${id}`, {
        method: "DELETE",
      });
  ...
  const handleStatusChange = async (id: string, status: Vendor["status"]) => {
    try {
      await fetch(`${API_BASE}/vendor/${id}/status`, {
        method: "PATCH",
        headers: {
    ...
    <div>
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center h-[300px]">
          ...
          <p className="text-[var(--text-secondary)]">Loading Vendors...</p>
        </div>
      )}
